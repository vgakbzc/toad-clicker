// @ts-nocheck
// machine-converted from break_eternity.js (https://github.com/Patashu/break_eternity.js)
/**
...
const MAX_SIGNIFICANT_DIGITS: number = 17; //Maximum number of digits of precision to assume in Number

const EXP_LIMIT: number = 9e15; //If we're ABOVE this value, increase a layer. (9e15 is close to the largest integer that can fit in a Number.)

const LAYER_DOWN: number = Math.log10(9e15);
const FIRST_NEG_LAYER: number = 1 / 9e15; //At layer 0, smaller non-zero numbers than this become layer 1 numbers with negative mag. After that the pattern continues as normal.

const NUMBER_EXP_MAX: number = 308; //The largest exponent that can appear in a Number, though not all mantissas are valid here.

const NUMBER_EXP_MIN: number = -324; //The smallest exponent that can appear in a Number, though not all mantissas are valid here.

const MAX_ES_IN_A_ROW: number = 5; //For default toString behaviour, when to swap from eee... to (e^n) syntax.

const DEFAULT_FROM_STRING_CACHE_SIZE: number = (1 << 10) - 1; // The default size of the LRU cache used to cache Decimal.fromString.

const powerOf10 = (function (): any {
...
const critical_headers: number[] = [2, Math.E, 3, 4, 5, 6, 7, 8, 9, 10];
const critical_tetr_values: number[][] = [
    [
...
const critical_slog_values: number[][] = [
    [
        // Base 2
...
let D = function D(value: any): any {
    return Decimal.fromValue_noAlloc(value);
} as any;

let FC = function FC(sign: any, layer: any, mag: any): any {
    return Decimal.fromComponents(sign, layer, mag);
} as any;

let FC_NN = function FC_NN(sign: any, layer: any, mag: any): any {
    return Decimal.fromComponents_noNormalize(sign, layer, mag);
} as any; // eslint-disable-next-line @typescript-eslint/no-unused-vars

function decimalPlaces(value, places: any): any {
    var len = places + 1;
...
function f_maglog10(n: any): any {
    return Math.sign(n) * Math.log10(Math.abs(n));
} //from HyperCalc source code

function f_gamma(n: any): any {
    if (!isFinite(n)) {
        return n;
    }
...
const _EXPN1: number = 0.36787944117144232159553; // exp(-1)

const OMEGA: number = 0.56714329040978387299997; // W(1, 0)
//from https://math.stackexchange.com/a/465183
// The evaluation can become inaccurate very close to the branch point
// Evaluates W(x, 0) if principal is true, W(x, -1) if principal is false

function f_lambertw(z: any): any {
    var tol =
...
export class Decimal {
    ...
    fromComponents(sign: any, layer: any, mag: any): any {
        this.sign = sign;
        this.layer = layer;
        this.mag = mag;
        this.normalize();
    ...
    fromComponents_noNormalize(sign: any, layer: any, mag: any): any {
        this.sign = sign;
        this.layer = layer;
        this.mag = mag;
        return this;
    ...
    fromMantissaExponent(mantissa: any, exponent: any): any {
        this.layer = 1;
    ...
    fromMantissaExponent_noNormalize(mantissa: any, exponent: any): any {
    ...
    mantissaWithDecimalPlaces(places: any): any {
    ...
    magnitudeWithDecimalPlaces(places: any): any {
        // https://stackoverflow.com/a/37425022
        if (isNaN(this.mag)) {
            return Number.NaN;
        }
    ...
    toStringWithDecimalPlaces(places: any): any {
        if (this.layer === 0) {
            if ((this.mag < 1e21 && this.mag > 1e-7) || this.mag === 0) {
                return (this.sign * this.mag).toFixed(places);
            }
    ...
    clampMin(min: any): any {
        return this.max(min);
    }

    clampMax(max: any): any {
        return this.min(max);
    }

    cmp_tolerance(value: any, tolerance: any): any {
        var decimal = D(value);
        return this.eq_tolerance(decimal, tolerance) ? 0 : this.cmp(decimal);
    }

    compare_tolerance(value: any, tolerance: any): any {
        return this.cmp_tolerance(value, tolerance);
    }

    eq_tolerance(value: any, tolerance: any): any {
    ...
    equals_tolerance(value: any, tolerance: any): any {
        return this.eq_tolerance(value, tolerance);
    }

    neq_tolerance(value: any, tolerance: any): any {
        return !this.eq_tolerance(value, tolerance);
    }

    notEquals_tolerance(value: any, tolerance: any): any {
        return this.neq_tolerance(value, tolerance);
    }

    lt_tolerance(value: any, tolerance: any): any {
        var decimal = D(value);
        return !this.eq_tolerance(decimal, tolerance) && this.lt(decimal);
    }

    lte_tolerance(value: any, tolerance: any): any {
        var decimal = D(value);
        return this.eq_tolerance(decimal, tolerance) || this.lt(decimal);
    }

    gt_tolerance(value: any, tolerance: any): any {
        var decimal = D(value);
        return !this.eq_tolerance(decimal, tolerance) && this.gt(decimal);
    }

    gte_tolerance(value: any, tolerance: any): any {
        var decimal = D(value);
        return this.eq_tolerance(decimal, tolerance) || this.gt(decimal);
    }
    ...
    layeradd(diff: any, base: any): any {
        var linear =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    linear_penta_root(degree: any): any {
        //1st-degree super root just returns its input
        if (degree == 1) {
            return this;
    ...
    ascensionPenalty(ascensions: any): any {
        if (ascensions === 0) {
            return new Decimal(this);
        }
    ...
    lessThanOrEqualTo(other: any): any {
    ...
    greaterThanOrEqualTo(other: any): any {
        return this.cmp(other) > -1;
    }
    ...
    static fromComponents(sign: any, layer: any, mag: any): any {
        return new Decimal().fromComponents(sign, layer, mag);
    }

    static fromComponents_noNormalize(sign: any, layer: any, mag: any): any {
        return new Decimal().fromComponents_noNormalize(sign, layer, mag);
    }

    static fromMantissaExponent(mantissa: any, exponent: any): any {
        return new Decimal().fromMantissaExponent(mantissa, exponent);
    }

    static fromMantissaExponent_noNormalize(mantissa: any, exponent: any): any {
    ...
    static fromDecimal(value: any): any {
    ...
    static fromValue_noAlloc(value: any): any {
        if (value instanceof Decimal) {
            return value;
        } else if (typeof value === "string") {
    ...
    static abs(value: any): any {
        return D(value).abs();
    }

    static neg(value: any): any {
        return D(value).neg();
    }
    ...
    static floor(value: any): any {
        return D(value).floor();
    }

    static ceil(value: any): any {
        return D(value).ceil();
    }

    static trunc(value: any): any {
    ...
    static plus(value: any, other: any): any {
        return D(value).add(other);
    }

    static sub(value: any, other: any): any {
    ...
    static minus(value: any, other: any): any {
        return D(value).sub(other);
    }

    static mul(value: any, other: any): any {
    ...
    static times(value: any, other: any): any {
        return D(value).mul(other);
    }

    static div(value: any, other: any): any {
    ...
    static recip(value: any): any {
        return D(value).recip();
    }

    static reciprocal(value: any): any {
        return D(value).recip();
    }

    static reciprocate(value: any): any {
        return D(value).reciprocate();
    }

    static mod(value: any, other: any): any {
        var floored =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    static modulo(value: any, other: any): any {
        var floored =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    static modular(value: any, other: any): any {
        var floored =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    static cmp(value: any, other: any): any {
        return D(value).cmp(other);
    }

    static cmpabs(value: any, other: any): any {
        return D(value).cmpabs(other);
    }

    static compare(value: any, other: any): any {
        return D(value).cmp(other);
    }
    ...
    static equals(value: any, other: any): any {
        return D(value).eq(other);
    }

    static neq(value: any, other: any): any {
        return D(value).neq(other);
    }

    static notEquals(value: any, other: any): any {
    ...
    static max(value: any, other: any): any {
        return D(value).max(other);
    }

    static min(value: any, other: any): any {
        return D(value).min(other);
    }

    static minabs(value: any, other: any): any {
        return D(value).minabs(other);
    }

    static maxabs(value: any, other: any): any {
    ...
    static clampMin(value: any, min: any): any {
        return D(value).clampMin(min);
    }

    static clampMax(value: any, max: any): any {
        return D(value).clampMax(max);
    }

    static cmp_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).cmp_tolerance(other, tolerance);
    }

    static compare_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).cmp_tolerance(other, tolerance);
    }

    static eq_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).eq_tolerance(other, tolerance);
    }

    static equals_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).eq_tolerance(other, tolerance);
    }

    static neq_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).neq_tolerance(other, tolerance);
    }

    static notEquals_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).notEquals_tolerance(other, tolerance);
    }

    static lt_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).lt_tolerance(other, tolerance);
    }

    static lte_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).lte_tolerance(other, tolerance);
    }

    static gt_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).gt_tolerance(other, tolerance);
    }

    static gte_tolerance(value: any, other: any, tolerance: any): any {
        return D(value).gte_tolerance(other, tolerance);
    }

    static pLog10(value: any): any {
        return D(value).pLog10();
    }

    static absLog10(value: any): any {
        return D(value).absLog10();
    }

    static log10(value: any): any {
        return D(value).log10();
    }

    static log(value: any, base: any): any {
        return D(value).log(base);
    }

    static log2(value: any): any {
        return D(value).log2();
    }

    static ln(value: any): any {
        return D(value).ln();
    }

    static logarithm(value: any, base: any): any {
        return D(value).logarithm(base);
    }

    static pow(value: any, other: any): any {
        return D(value).pow(other);
    }

    static pow10(value: any): any {
        return D(value).pow10();
    }

    static root(value: any, other: any): any {
        return D(value).root(other);
    }

    static factorial(value: any, _other: any): any {
        return D(value).factorial();
    }

    static gamma(value: any, _other: any): any {
        return D(value).gamma();
    }

    static lngamma(value: any, _other: any): any {
        return D(value).lngamma();
    }

    static exp(value: any): any {
        return D(value).exp();
    }

    static sqr(value: any): any {
        return D(value).sqr();
    }

    static sqrt(value: any): any {
        return D(value).sqrt();
    }

    static cube(value: any): any {
        return D(value).cube();
    }

    static cbrt(value: any): any {
        return D(value).cbrt();
    }

    static tetrate(value: any): any {
        var height =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : 2;
    ...
    static iteratedexp(value: any): any {
        var height =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : 2;
    ...
    static iteratedlog(value: any): any {
        var base =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : 10;
    ...
    static layeradd10(value: any, diff: any): any {
        var linear =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    static layeradd(value: any, diff: any): any {
        var base =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : 10;
    ...
    static lambertw(value: any, principal: any): any {
        return D(value).lambertw(principal);
    }

    static ssqrt(value: any): any {
        return D(value).ssqrt();
    }

    static linear_sroot(value: any, degree: any): any {
        return D(value).linear_sroot(degree);
    }

    static pentate(value: any): any {
        var height =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : 2;
    ...
    static penta_log(value: any): any {
        var base =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : 10;
    ...
    static linear_penta_root(value: any, degree: any): any {
        return D(value).linear_penta_root(degree);
    }

    static sin(value: any): any {
        return D(value).sin();
    }

    static cos(value: any): any {
        return D(value).cos();
    }

    static tan(value: any): any {
        return D(value).tan();
    }

    static asin(value: any): any {
        return D(value).asin();
    }

    static acos(value: any): any {
        return D(value).acos();
    }

    static atan(value: any): any {
        return D(value).atan();
    }

    static sinh(value: any): any {
        return D(value).sinh();
    }

    static cosh(value: any): any {
        return D(value).cosh();
    }

    static tanh(value: any): any {
        return D(value).tanh();
    }

    static asinh(value: any): any {
        return D(value).asinh();
    }

    static acosh(value: any): any {
        return D(value).acosh();
    }

    static atanh(value: any): any {
    ...
    efficiencyOfPurchase(cost: any, currentRpS: any, deltaRpS: any): any {
    ...
    static randomDecimalForTesting(maxLayers: any): any {
    ...
    static slog_critical(base: any, height: any): any {
        //TODO: for bases above 10, revert to old linear approximation until I can think of something better
        if (base > 10) {
            return height - 1;
        }
    ...
    static tetrate_critical(base: any, height: any): any {
        return Decimal.critical_section(base, height, critical_tetr_values);
    }

    static critical_section(base: any, height: any, grid: any): any {
    ...
    static excess_slog(value: any, base: any): any {
        var linear =
            arguments.length > 2 && arguments[2] !== undefined
                ? arguments[2]
                : false;
    ...
    static increasingInverse(func: any): any {
        var decreasing =
            arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : false;
...
// Copy all prototype methods to the constructor to support static-method-style calls
// (the original Babel output placed all methods on prototype, but code accesses them as Decimal.method())
